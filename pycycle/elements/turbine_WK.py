...
class CorrectedInputsCalc(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('Wp', val=30.0, units='lbm/s', desc='corrected mass flow')
        self.add_output('Np', val=100., units='rpm', desc='corrected shaft speed')
    ...
    def compute(self, inputs, outputs):
        ...
        try:
            ...
            raise AnalysisError(
                'Bad values flow states in {}: T={}  P={}'.format(self.pathname, inputs['Tt'], inputs['Pt']))
...
class eff_poly_calc(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials('eff_poly',
                              '*')  # self.declare_partials('Rt','Cp',val=1.0)  # self.declare_partials('Rt','Cv',val=-1.0)
...
class PressureDrop(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('Pt_in', val=5.0, units='psi', desc='Inlet total pressure')

        # outputs
        self.add_output('Pt_out', shape=1, units='psi', desc='Exit total pressure', lower=1e-3)
...
class EnthalpyDrop(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('ht_out_ideal', val=10.0, units='Btu/lbm', desc='incoming ideal enthalpy')
        self.add_input('eff', val=0.8, desc='isentropic efficiency')

        # outputs
        self.add_output('ht_out', shape=1, units='Btu/lbm', desc='actual enthalpy')

        self.declare_partials('ht_out', ['ht_in', 'ht_out_ideal', 'eff'])

    def compute(self, inputs, outputs):
        outputs['ht_out'] = inputs['ht_in'] - (inputs['ht_in'] - inputs['ht_out_ideal']) * inputs['eff']
...
class BleedSplitter(om.ExplicitComponent):
    """Computes bleed splitting in working and non working"""

    def initialize(self):
        self.options.declare('bleed_names', types=Iterable, desc='list of names for the bleed ports', default=[])
    ...
    def setup(self):
        ...
        bleeds = self.options['bleed_names']
        for BN in bleeds:
            self.add_input(BN + ':W', val=0.0, units='lbm/s', desc='bleed mass flow rate')
            self.add_input(BN + ':WF', val=0.0, desc='working factor')

            self.add_output(BN + '_W' + ':W', val=0.0, units='lbm/s', desc='Working bleed mass flow rate')
            self.add_output(BN + '_NW' + ':W', val=0.0, units='lbm/s', desc='Non Working bleed mass flow rate')

            self.declare_partials(BN + '_W' + ':W', [BN + ':W', BN + ':WF'])
            self.declare_partials(BN + '_NW' + ':W', [BN + ':W', BN + ':WF'])
    ...
    def compute(self, inputs, outputs):
        ...
        for BN in bleeds:
            W = inputs[BN + ':W']
            WF = inputs[BN + ':WF']
            outputs[BN + '_W' + ':W'] = W * WF
            outputs[BN + '_NW' + ':W'] = W * (1 - WF)
...
class BleedPressure(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('Pt_in', val=0.0, units='psi', desc='turbine entrance pressure')
        self.add_input('Pt_out', val=0.0, units='psi', desc='turbine exit pressure')
        # self.add_input('W_in', val=0.0, units='lbm/s',
        #                desc='turbine entrance mass flow rate')

        # bleed inputs and outputs
        for BN in bleeds:
            self.add_input(BN + ':frac_P', val=0.0, desc='fraction of pressure drop where bleed flow introduced')
            self.add_input(BN + ':W', val=0.0, units='lbm/s', desc='bleed mass flow rate')

            self.add_output(BN + ':Pt', shape=1, units='psi', desc='pressure of incomming bleed flow', lower=1e-3)
...
class EnthalpyAndPower(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('ht_in', val=10.0, units='Btu/lbm', desc='entrance enthalpy')
        self.add_input('ht_out_ideal', val=10.0, units='Btu/lbm', desc='ideal exit enthalpy')
        self.add_input('eff', val=1.0, desc='turbine efficiency')
        self.add_input('Nmech', val=1000.0, units='rpm', desc='shaft speed')

        self.add_output('ht_out_b4bld', shape=1, units='Btu/lbm', desc='downstream enthalpy')
        self.add_output('ht_out', shape=1, units='Btu/lbm', desc='downstream enthalpy')
        ...
        for BN in bleeds:
            ...
            self.add_input(BN_W, val=0.0, units='lbm/s', desc='bleed mass flow rate')
            self.add_input(BN_ht, val=0.0, units='Btu/lbm', desc='bleed total enthalpy')
            self.add_input(BN_ht_ideal, val=0.0, units='Btu/lbm', desc='ideally expanded bleed total enthalpy')
...
class Turbine(Element):
    ...
    def initialize(self):
        ...
        self.options.declare('map_interp_method', default='slinear', desc='Method to use for map interpolation. \
        ...
        self.default_des_od_conns = [  # (design src, off-design target)
            ('s_Wp', 's_Wp'), ('s_PR', 's_PR'), ('s_eff', 's_eff'), ('s_Np', 's_Np'), ('Fl_O:stat:area', 'area')]
    ...
    def pyc_setup_output_ports(self):
        ...
        self.bleed_element_list = []
        for bleed_name in bleeds:
            self.bleed_element_list.append(self.Fl_I_data[bleed_name])
            self.copy_flow(bleed_name, f'{bleed_name}_W')
            self.copy_flow(bleed_name, f'{bleed_name}_NW')

        self.bld_add = ThermoAdd(method=thermo_method, mix_names=bleeds, mix_mode='flow',
                                 thermo_kwargs={'spec': thermo_data, 'inflow_composition': inflow_composition,
                                                'mix_composition': self.bleed_element_list})
    ...
    def setup(self):
        ...
        self.add_subsystem('corrinputs', CorrectedInputsCalc(),
                           promotes_inputs=['Nmech', ('W_in', 'Fl_I:stat:W'), ('Pt', 'Fl_I:tot:P'),
                                            ('Tt', 'Fl_I:tot:T')], promotes_outputs=['Np', 'Wp'])

        turb_map = TurbineMap(map_data=map_data, design=designFlag, interp_method=interp_method, extrap=map_extrap)
        if designFlag:
            self.add_subsystem('map', turb_map, promotes_inputs=['Np', 'Wp', 'PR', 'eff'],
                               promotes_outputs=['s_PR', 's_Wp', 's_eff', 's_Np'])
        else:
            self.add_subsystem('map', turb_map, promotes_inputs=['Np', 'Wp', 's_PR', 's_Wp', 's_eff', 's_Np'],
                               promotes_outputs=['PR', 'eff'])

        # Calculate pressure drop across turbine
        self.add_subsystem('press_drop', PressureDrop(), promotes_inputs=['PR', ('Pt_in', 'Fl_I:tot:P')])

        # Calculate ideal flow station properties
        ideal_flow = Thermo(mode='total_SP', method=thermo_method,
        ...
        # Calculate working and non working bleeds
        blds_splitter = BleedSplitter(bleed_names=bleeds)
        self.add_subsystem('blds_splitter', blds_splitter,
                           promotes_inputs=[(f'{BN}:W', f'{BN}:stat:W') for BN in bleeds] + [f'{BN}:WF' for BN in
                                                                                             bleeds],
                           promotes_outputs=['*'])

        # Station 45
        bld_add_WK = ThermoAdd(method=thermo_method, mix_names=bleeds, mix_mode='flow',
                               thermo_kwargs={'spec': thermo_data, 'inflow_composition': composition,
                                              'mix_composition': self.bleed_element_list})

        self.add_subsystem('bld_add_WK', bld_add_WK,
                           promotes_inputs=['Fl_I:stat:W', 'Fl_I:tot:composition', 'Fl_I:tot:h'] +
                                           [(f'{BN}:W', f'{BN}_W:W') for BN in bleeds] + [(f'{BN}:h', f'{BN}:tot:h') for BN in
                                                                             bleeds] + [
                                               (f'{BN}:composition', f'{BN}:tot:composition') for BN in bleeds],
                           promotes_outputs=[('Wout', 'W_45'), ('mass_avg_h', 'ht_45'),
                                             ('composition_out', 'composition_45')])

        st_45 = Thermo(mode='total_hP', fl_name='st_45:tot', method=thermo_method,
                       thermo_kwargs={'composition': composition, 'spec': thermo_data})
        self.add_subsystem('st_45', st_45, promotes_outputs=['st_45:tot:*'],
                           promotes_inputs=[('composition', 'composition_45'), ('h', 'ht_45'), ('P', 'Fl_I:tot:P')])

        # Calculate ideal flow station properties
        st_455_ideal = Thermo(mode='total_SP', fl_name='st_455_ideal:tot', method=thermo_method,
                              thermo_kwargs={'composition': composition, 'spec': thermo_data})
        self.add_subsystem('st_455_ideal', st_455_ideal, promotes_outputs=['st_455_ideal:tot:*'],
                           promotes_inputs=[('S', 'st_45.S'), ('composition', 'composition_45')])

        self.connect("press_drop.Pt_out", "st_455_ideal.P")

        # Calculate enthalpy at station 455
        self.add_subsystem('enthalpy_drop', EnthalpyDrop(),
                           promotes_inputs=['eff', ('ht_in', 'ht_45'), ('ht_out_ideal', 'st_455_ideal:tot:h')],
                           promotes_outputs=[('ht_out', 'ht_455')])

        st_455 = Thermo(mode='total_hP', fl_name='st_455:tot', method=thermo_method,
                        thermo_kwargs={'composition': composition, 'spec': thermo_data})
        self.add_subsystem('st_455', st_455, promotes_outputs=['st_455:tot:*'],
                           promotes_inputs=[('composition', 'composition_45'), ('h', 'ht_455')])
        self.connect('st_455_ideal:tot:P', 'st_455.P')

        # Calculate outlet station
        bld_add_NWK = ThermoAdd(method=thermo_method, mix_names=bleeds, mix_mode='flow',
                                thermo_kwargs={'spec': thermo_data, 'inflow_composition': composition,
                                               'mix_composition': self.bleed_element_list})

        self.add_subsystem('bld_add_NWK', bld_add_NWK,
                           promotes_inputs=[('Fl_I:stat:W', 'W_45'), ('Fl_I:tot:composition', 'composition_45'),
                                            ('Fl_I:tot:h', 'ht_455')] + [(f'{BN}:W', f'{BN}_NW:W') for BN in bleeds] + [
                                               (f'{BN}:h', f'{BN}:tot:h') for BN in bleeds] + [
                                               (f'{BN}:composition', f'{BN}:tot:composition') for BN in bleeds],
                           promotes_outputs=[('Wout', 'W_50'), ('mass_avg_h', 'ht_50'),
                                             ('composition_out', 'composition_50')])
        st_50 = Thermo(mode='total_hP', fl_name='st_50:tot', method=thermo_method,
                       thermo_kwargs={'composition': composition, 'spec': thermo_data})
        self.add_subsystem('st_50', st_50, promotes_outputs=['st_50:tot:*'],
                           promotes_inputs=[('composition', 'composition_50'), ('h', 'ht_50'), ('P', 'st_455:tot:P')])
        ...
        self.add_subsystem('bld_add', self.bld_add,
                           promotes_inputs=['Fl_I:stat:W', 'Fl_I:tot:composition'] + [(f'{BN}:W', f'{BN}:stat:W') for BN
                                                                                      in bleeds] + [
                                               (f'{BN}:composition', f'{BN}:tot:composition') for BN in bleeds],
        ...
        for BN in bleeds:
            ...
            inflow = Thermo(mode='total_hP', method=thermo_method,
                            thermo_kwargs={'composition': self.Fl_I_data[BN], 'spec': thermo_data})
            ...
            ideal = Thermo(mode='total_SP', method=thermo_method,
                           thermo_kwargs={'composition': self.Fl_I_data[BN], 'spec': thermo_data})
            self.add_subsystem(f'{BN}_ideal', ideal, promotes_inputs=[('composition', BN + ":tot:composition")])
        ...
        self.add_subsystem('pwr_turb', EnthalpyAndPower(bleed_names=bleeds),
                           promotes_inputs=['Nmech', 'eff', 'W_out', ('W_in', 'Fl_I:stat:W'),
                                            ('ht_in', 'Fl_I:tot:h')] + [(BN + ':W', BN + ':stat:W') for BN in
                                                                        bleeds] + [(BN + ':ht', BN + ':tot:h') for BN in
                                                                                   bleeds] + [
                                               (BN + ':ht_ideal', BN + '_ideal.h') for BN in bleeds],
        ...
        real_flow_b4bld = Thermo(mode='total_hP', fl_name="Fl_O_b4bld:tot", method=thermo_method,
                                 thermo_kwargs={'composition': composition, 'spec': thermo_data})
        ...
        real_flow = Thermo(mode='total_hP', fl_name="Fl_O:tot", method=thermo_method,
                           thermo_kwargs={'composition': composition, 'spec': thermo_data})
        self.add_subsystem('real_flow', real_flow, promotes_outputs=['Fl_O:tot:*'])
        ...
        if statics:
            if designFlag:
                #   SetStaticMN
                out_stat = Thermo(mode='static_MN', fl_name="Fl_O:stat", method=thermo_method,
                                  thermo_kwargs={'composition': composition, 'spec': thermo_data})
                self.add_subsystem('out_stat', out_stat, promotes_inputs=['MN'], promotes_outputs=['Fl_O:stat:*'])
                ...
                #   SetStaticArea
                out_stat = Thermo(mode='static_A', fl_name="Fl_O:stat", method=thermo_method,
                                  thermo_kwargs={'composition': composition, 'spec': thermo_data})
                self.add_subsystem('out_stat', out_stat, promotes_inputs=['area'], promotes_outputs=['Fl_O:stat:*'])
                self.connect('bld_add.composition_out', 'out_stat.composition')
                self.connect('Fl_O:tot:S', 'out_stat.S')
                self.connect('Fl_O:tot:h', 'out_stat.ht')
                self.connect('W_out', 'out_stat.W')
            ...
            self.set_order(
                ['in_flow', 'corrinputs', 'map', 'press_drop', 'ideal_flow'] + bleeds + ['blds_splitter', 'bld_add_WK',
                                                                                         'st_45', 'st_455_ideal',
                                                                                         'enthalpy_drop', 'st_455',
                                                                                         'bld_add_NWK', 'st_50'] + [
                    'bld_add', 'blds'] + bleed_names2 + ['pwr_turb', 'real_flow_b4bld', 'eff_poly_calc', 'real_flow',
                                                         'out_stat'])

        else:
            self.add_subsystem('W_passthru', PassThrough('W_out', 'Fl_O:stat:W', 1.0, units="lbm/s"), promotes=['*'])
            self.set_order(['in_flow', 'corrinputs', 'map', 'press_drop', 'ideal_flow'] + bleeds + ['bld_add',
                                                                                                    'blds'] + bleed_names2 + [
